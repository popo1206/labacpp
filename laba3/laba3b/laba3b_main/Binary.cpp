...
namespace laba3{
   ...
   std::ostream& operator << (std::ostream& s, const Signal &a){
        for (int i=0;i<a.n;i++){
            if (a.BSignal[i].level==true) std::cout<<"-";
            else std::cout<<"_";
            std::cout<<"("<< static_cast<int>(a.BSignal[i].duration)<<")";
        }
        std::cout<<std::endl;
       return s;
    }
    void input_lev(bool& b){
     ...
     std::istream& operator >> (std::istream& s, Signal &a){
        int k;
        std::cout<<"Input number of singals from 1 to "<<a.SZ<<std::endl;
         std::cin>>k;
        for (int i=0;i<k;i++){
            bool b;
            int dur;
            ...
            std::cin>>dur;
            try{
                if (dur>255) throw std::runtime_error("Too long durtion");
                if (a.n!=a.SZ){
                    a.BSignal[a.n].level=b;
                    a.BSignal[a.n].duration=dur;
                    a.n++;
                }
            }
    ...
    Signal &  Signal:: operator ! (){// инверсия
        for (int i  = 0; i < n; i++){
            BSignal[i].level=!BSignal[i].level;
        }
        return *this;
    }
    Signal& Signal:: operator+=(Signal &c){
    ...
    Signal & Signal:: operator() (int t, Signal& c){
        int t0=t;
        int i0=0;
    ...
    Signal & Signal::operator() (int t,int dur){
        int t_begin=t;
        int t_end=t+dur;
    ...
    Signal & Signal::operator =(Signal&s1){
        n=s1.getn();
        for (int i=0;i<n;i++){
            BSignal[i].level=s1.getBSignal(i).level;
             BSignal[i].duration=s1.getBSignal(i).duration;
        }
        return *this;
    }
